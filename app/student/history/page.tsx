    ...
    <StudentLayout activeTab="history">
      <div className="space-y-6">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Review your academic progress and achievements</p>
        </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-chart-1">{averageGrade}%</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-chart-2">{completedAssignments.length}</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-chart-3">{attendanceRate}%</p>
                </div>
            ...
            <CardContent className="p-4">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-2xl font-bold text-chart-4">{totalClasses}</p>
                </div>
                  ...
                  {completedAssignments.map((assignment) => (
                    <div key={assignment.id} className="p-3 bg-muted rounded-lg">
                      <div className="flex items-start justify-between">
                        ...
                        <Badge variant={assignment.grade && assignment.grade >= 80 ? "default" : "secondary"}>
                          {assignment.grade}%
                        </Badge>
                      </div>
                    </div>
            ...
            <CardContent>
              <div className="space-y-3">
                {[...attendance.slice(0, 5), ...assignments.slice(0, 3)]
                  .sort((a, b) => {
                    const dateA = "date" in a ? a.date : a.submittedAt || a.dueDate
                    const dateB = "date" in b ? b.date : b.submittedAt || b.dueDate
                    ...
                    <div key={index} className="flex items-center space-x-3 p-2 rounded-lg bg-muted">
                      ...
                      <div className="flex-1">
                        <p className="text-sm font-medium">
                          {"subject" in item ? `${item.subject} Class` : item.title}
                        </p>
                        <p className="text-xs text-muted-foreground">
                          {"date" in item ? item.date : item.submittedAt || item.dueDate}
                        </p>
                      </div>
                      {"status" in item ? (
                        <Badge variant={item.status === "present" ? "default" : "destructive"} className="text-xs">
                          {item.status}
                        </Badge>
                      ) : (
